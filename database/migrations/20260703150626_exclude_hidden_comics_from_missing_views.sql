CREATE OR REPLACE VIEW `v_comics_missing_cast` AS
SELECT `c`.`id`
FROM `Comic` `c`
WHERE NOT EXISTS (
...
CREATE OR REPLACE VIEW `v_comics_missing_location` AS
SELECT `c`.`id`
FROM `Comic` `c`
WHERE NOT EXISTS (
    SELECT 1 FROM `Occurrence` `o`
...
CREATE OR REPLACE VIEW `v_comics_missing_storyline` AS
SELECT `c`.`id`
FROM `Comic` `c`
WHERE NOT EXISTS (
    SELECT 1 FROM `Occurrence` `o`
...
CREATE OR REPLACE VIEW `v_comics_missing_title` AS
SELECT `c`.`id`
FROM `Comic` `c`
...
CREATE OR REPLACE VIEW `v_comics_missing_tagline` AS
SELECT `c`.`id`
FROM `Comic` `c`
